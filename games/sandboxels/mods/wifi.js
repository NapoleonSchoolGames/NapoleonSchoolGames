  ...
  elements.wifi = {
    ...
    tick: function(pixel) {
      ...
      pixel._correspondingWifi = currentPixels.filter(function(pixelToCheck) {
        return (
          pixelToCheck !== pixel && ["wifi", "receiver"].includes(pixelToCheck.element) &&
          ...
          if (newPixel) {
            if (!newPixel.chargeCD) {
              for (var j in adjacentCoords) {
                ...
                if (pixelAdjacentToWifi) {
                  pixelAdjacentToWifi.charge = 1
                };
              };
            }
          }
          ...
          if (newPixel) {
            if (!newPixel.chargeCD) {
              for (var j in adjacentCoords) {
                j = parseInt(j);
                var pixelAdjacentToWifi = pixelMap[newPixel.x + adjacentCoords[j][0]]?. [newPixel.y + adjacentCoords[j][1]];
                if (pixelAdjacentToWifi && elements[pixelAdjacentToWifi.element].conduct) {
                  pixelAdjacentToWifi.charge = 1
                };
              };
            }
