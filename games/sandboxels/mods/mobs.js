function whenAvailable(names, callback) {
  ...
  if (enabledMods.includes(runAfterAutogenMod) && enabledMods.includes(explodeAtPlusMod) && enabledMods.includes(libraryMod)) {
	whenAvailable(["runAfterAutogen", "explodeAtPlus"], function() {
	  ...
	  if (typeof(settings.creeperSpawning) === "undefined") {
		style.innerHTML = '.creeperStatus { color: #E11; text-decoration: none; }';
	  } else {
		if (!settings.creeperSpawning) {
		  style.innerHTML = '.creeperStatus { color: #E11; text-decoration: none; }';
		} else if (settings.creeperSpawning) {
		  style.innerHTML = '.creeperStatus { color: #1E1; text-decoration: none; }';
		};
	  };
	  document.getElementsByTagName('head')[0].appendChild(style);
  
	  function coordPyth(xA, yA, xB, yB) {
	  ...
	  function angelicUpwardVelocity(pixel, x, y, radius, fire, smoke, power) {
		...
		if (enabledMods.includes("mods/velocity.js")) {
		  ...
		  if (!elements[pixel.element].excludeRandom) {
			...
			pixel.vy = 0 - Math.abs(Math.round((pixel.vy | 0) + Math.sin(angle) * (radius * power / 4)) + 4);
	  ...
	  function hellExplosionFire(pixel, x, y, radius, fire, smoke, power, damage) {
	  ...
	  function updateCreeperPreferences() {
		if (settings.creeperSpawning) {
		  if (typeof(randomEvents.creeper) !== "function") {
			randomEvents.creeper = function() {
			  var amount = Math.floor((Math.random() * maximumCreeperTries) + minimumCreeperTries);
		  ...
		  if (randomEvents.creeper) {
			delete randomEvents.creeper
		  };
		};
	  };
  
	  function toggleCreeperSpawning() {
		if (settings.creeperSpawning != true) {
		  setSetting("creeperSpawning", true);
		  updateCreeperPreferences();
		  document.getElementById("creeperStatusStylesheet").innerHTML = '.creeperStatus { color: #1E1; text-decoration: underline; }';
		} else {
		  setSetting("creeperSpawning", false);
		  updateCreeperPreferences();
	  ...
	  function breakPixel(pixel, changetemp = false, defaultBreakIntoDust = false) {
		...
		if (typeof(info.breakInto) === "undefined") {
		  if (defaultBreakIntoDust) {
			if (Math.random() < defaultBreakIntoDust) {
			  changePixel(pixel, "dust", changetemp)
			};
		  };
	  ...
	  function nothingThereBulletMovement(pixel, x, y) {
		if (!tryMove(pixel, x, y)) {
		  if (!isEmpty(x, y, true)) {
			...
			if (typeof(newInfo.state) === "undefined" && newElement !== pixel.element) {
	  ...
	  elements.spawner = {
		...
		tick: function(pixel) {
		  ...
		  if (Array.isArray(pixel.spawn)) {
			pixel.spawn = pixel.spawn.filter(function(e) {
	  ...
	  if (typeof(settings.zombieSpawning) === "undefined") {
		style.innerHTML = '.zombieStatus { color: #E11; text-decoration: none; }';
	  } else {
		if (!settings.zombieSpawning) {
		  style.innerHTML = '.zombieStatus { color: #E11; text-decoration: none; }';
		} else if (settings.zombieSpawning) {
	  ...
	  function updateZombiePreferences() {
		if (settings.zombieSpawning) {
		  if (typeof(randomEvents.zombie) !== "function") {
			randomEvents.zombie = function() {
			  var amount = Math.floor((Math.random() * maximumZombieTries) + minimumZombieTries);
		  ...
		  if (randomEvents.zombie) {
			delete randomEvents.zombie
		  };
		};
	  };
  
	  function toggleZombieSpawning() {
		if (settings.zombieSpawning != true) {
		  setSetting("zombieSpawning", true);
		  updateZombiePreferences();
		  document.getElementById("zombieStatusStylesheet").innerHTML = '.zombieStatus { color: #1E1; text-decoration: underline; }';
		} else {
		  setSetting("zombieSpawning", false);
		  updateZombiePreferences();
	  ...
	  if (typeof(settings.skeletonSpawning) === "undefined") {
		style.innerHTML = '.skeletonStatus { color: #E11; text-decoration: none; }';
	  } else {
		if (!settings.skeletonSpawning) {
		  style.innerHTML = '.skeletonStatus { color: #E11; text-decoration: none; }';
		} else if (settings.skeletonSpawning) {
	  ...
	  function updateSkeletonPreferences() {
		if (settings.skeletonSpawning) {
		  if (typeof(randomEvents.skeleton) !== "function") {
			randomEvents.skeleton = function() {
			  var amount = Math.floor((Math.random() * maximumSkeletonTries) + minimumSkeletonTries);
		  ...
		  if (randomEvents.skeleton) {
			delete randomEvents.skeleton
		  };
		};
	  };
  
	  function toggleSkeletonSpawning() {
		if (settings.skeletonSpawning != true) {
		  setSetting("skeletonSpawning", true);
		  updateSkeletonPreferences();
		  document.getElementById("skeletonStatusStylesheet").innerHTML = '.skeletonStatus { color: #1E1; text-decoration: underline; }';
		} else {
		  setSetting("skeletonSpawning", false);
		  updateSkeletonPreferences();
	  ...
	  elements.zombie_body = {
		...
		reactions: {
		  ...
		  "radiation": {
			"elem1": ["ash", "rotten_meat", "rotten_meat"],
			"chance": 0.4
		  },
		  "plague": {
			"elem1": "plague",
				  ...
				  if (!isEmpty(nX, nY, true)) {
					...
					if (enemyHumanoidArray.includes(newElement)) {
					  ...
					  if (!newPixel.dead) {
						...
						if (coordPyth(pX, pY, nX, nY) <= 1.5) {
						  if (Math.random() < 1 / 3) {
							if (Math.random() < 1 / 3) {
							  changePixel(newPixel, "zombie_blood", false);
							} else {
		  ...
		  tick: function(pixel) {
			if (!pixel.hissing) {
			  ...
			  if ((pixelTicks - pixel.start) % 3 == 0) {
				if (!isEmpty(pixel.x, pixel.y - 1, true)) {
				  ...
				  if (headPixel.element == "angelic_creeper_head") {
					if (tryMove(headPixel, pixel.x, pixel.y - 2)) {
					  if (isEmpty(pixel.x, pixel.y - 1)) {
			...
			if (pixel.burning) {
			  ...
			  if (pixelTicks - pixel.burnStart > 30) {
				...
				explodeAtPlus(pixel.x, pixel.y, explosionRadius, "fire", "smoke", null, angelicUpwardVelocity);
		...
		tick: function(pixel) {
		  ...
		  if (pixel.burning) {
			...
			if (pixelTicks - pixel.burnStart > 30) {
			  ...
			  explodeAt(pixel.x, pixel.y, explosionRadius, "fire,dynamite");
	  ...
	  runAfterLoad(function() {
		if (typeof(badPixels) === "object") {
		  badPixels.nothing_there_phase_1 = {
			panicIncrease: 1,
			panicIncreaseChance: 1
		  }
		  badPixels.nothing_there_phase_2 = {
			panicIncrease: 1,
			panicIncreaseChance: 1
		  }
		  badPixels.nothing_there_phase_3_body = {
			panicIncrease: 1,
			panicIncreaseChance: 1
		  }
		  badPixels.nothing_there_phase_3_head = {
			panicIncrease: 1,
			panicIncreaseChance: 1
		  }
		}
	  ...
	  elements.arrow = {
		...
		tick: function(pixel) {
		  ...
		  if (pixel.attached) {
			...
			var dir = pixel.flipX ? -1 : 1;
			if (Math.random() < (1 / (pixel.speed ** 1.585))) {
			  pixel.fall++;
			};
			...
			for (i = 0; i < pixel.speed; i++) {
			  ...
			  if (!isEmpty(pixel.x + dir, pixel.y, true)) {
				...
				if (Math.random() < swapChance && pixel.penetrateCounter > 0) {
				  ...
				  if (!arrowAltTb(otherPixel, breakMult)) {
					pixel.attached = true;
					pixel.speed = 0;
					pixel.fall = 0;
					pixel.attachOffsets = [dir, 0];
