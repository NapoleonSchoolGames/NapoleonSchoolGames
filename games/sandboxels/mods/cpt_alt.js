    ...
    if (!tryMove(pixel, x, y)) {
      ...
      if (!isEmpty(x, y, true)) {
        ...
        if (newElement == pixel.element) {
          ...
          breakCircle(newPixel.x, newPixel.y, pixel.breakAroundRadius, false, false, false);
          swapPixels(pixel, newPixel);
          return true;
        };
      } else {
    ...
    tick: function(pixel) {
      ...
      if (alt_sussyKey !== null) {
        switch (alt_sussyKey) {
          case "W":
            if (isAlt) {
              controllablePixelTryCreatePixelNullCheck(userElement, pixel.x, pixel.y - 1)
            } else {
              for (move = 0; move < pixel.speed; move++) {
                actTryMove(pixel, pixel.x, pixel.y - 1)
              }
            };
            if (!isShift) {
              alt_sussyKey = null;
          ...
          case "A":
            if (isAlt) {
              ...
              for (move = 0; move < pixel.speed; move++) {
                actTryMove(pixel, pixel.x - 1, pixel.y)
              }
            };
            if (!isShift) {
              alt_sussyKey = null;
