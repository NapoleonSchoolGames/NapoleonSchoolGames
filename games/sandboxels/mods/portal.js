...
if (enabledMods.includes(onTryMoveIntoMod) && enabledMods.includes(libraryMod)) {
  ...
  elements.portal_in = {
      ...
      onTryMoveInto: function(pixel, otherPixel) {
        ...
        var offset = {
          x: pixel.x - otherPixel.x,
          y: pixel.y - otherPixel.y
        }; //teleport destination's offset, inverted by subtraction
      ...
      tick: function(pixel) {
        ...
        for (i = 0; i < pixel._correspondingPortals.length; i++) {
          pixel._correspondingPortals[i] = {
            x: pixel._correspondingPortals[i].x,
            y: pixel._correspondingPortals[i].y
          };
        };
