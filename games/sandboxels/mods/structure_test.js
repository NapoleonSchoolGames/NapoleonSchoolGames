...
if (!enabledMods.includes(libraryMod)) {
  ...
  oldRoom = [
    ["brick", "brick", "brick", "brick", "brick", "brick", "brick", "glass", "glass", "glass", "glass", "glass", "brick", "brick", "brick", "brick", "brick", "brick", "brick"],
  ...
  function loadPixelRowFromArray(pixelArray, centerX, centerY, evenLengthBiasedLeft = true, doOverwrite = true) {
    ...
    var leftmostOffset = (evenLengthBiasedLeft ? Math.floor(0 - ((arrayLength - 1) / 2)) : Math.ceil(0 - ((arrayLength - 1) / 2)))
    ...
    for (i = 0; i < arrayLength; i++) {
      ...
      if (!arrayLoaderVoids.includes(newElement)) {
        createPixel(newElement, x, y);
      }
  ...
  elements.glass.hardness = 0.25,
    ...
    elements.crumbling_concrete = {
      ...
      tick: function(pixel) {
        ...
        var supportCondition10 = (canSupportWithEdge(px, py + 1) && canSupportWithEdge(px, py - 1))
  ...
  elements.glass_shard.reactions.radiation = {
    "elem1": "rad_glass_shard",
    "chance": 0.33
  };
  ...
  elements.molten_glass.reactions.radiation = {
    "elem1": "molten_rad_glass",
    "chance": 0.33
  };
  ...
  elements.building_1_seed = {
    tick: function(pixel) {
      ...
      if (!tryMove(pixel, pixel.x, pixel.y + 1)) {
        var randomHeight = 13 + Math.floor(Math.random() * (8 + 1))
