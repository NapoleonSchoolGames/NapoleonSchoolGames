...
if (enabledMods.includes(libraryMod)) {
  if (urlParams.get('liquidAmount') != null) {
    liquidAmount = urlParams.get('liquidAmount')
    if (isNaN(liquidAmount) || liquidAmount === "" || liquidAmount === null) {
      liquidAmount = 10
    }
  ...
  if (urlParams.get('makeLiquidString') !== null) {
    makeLiquidString = true
  } else {
    makeLiquidString = false
  }

  const initialArrayL = ["m", "n", "p", "t", "ch", "k", "b", "d", "j", "g", "f", "th", "s", "sh", "h", "l", "r", "y", "w", "z", "sp", "st", "sk", "sl", "spl", "stl", "skl", "sr", "spr", "str", "skr", "sl", "fl", "fr", "pl", "pr", "tl", "tr", "kl", "kr", "shr", "fl", "fr", "thr"]
  const vowelArrayL = ["a", "e", "i", "o", "u", "ay", "ee", "ie", "oa", "ew", "oo", "oi", "ow"]
  const medialArrayL = ["m", "n", "p", "t", "k", "b", "d", "g", "f", "th", "s", "sh", "h", "l", "r", "y", "z", "sp", "st", "sk", "sl", "spl", "stl", "skl", "sr", "spr", "str", "skr", "sl", "fl", "fr", "pl", "pr", "tl", "tr", "kl", "kr", "shr", "fl", "fr", "thr"]
  const finalArrayL = ["m", "n", "p", "t", "k", "b", "d", "g", "f", "th", "s", "sh", "l", "r", "y", "z", "sp", "st", "sk", "sl", "spl", "stl", "skl", "sr", "spr", "str", "skr", "pl", "pr", "tl", "tr", "bl", "vr"]
  ...
  for (i = 0; i < liquidAmount; i++) {
    ...
    if (makeLiquidString == true) {
      ...
      if (conducts == true) {
        liquidString = liquidString + `conduct: ${conductivity * liquidConductivityAdjust},\n    `
      }
      if (burns == true) {
        liquidString = liquidString + `burn: ${burn},\n    burnTime: ${burnTime},\n    `
      }
      liquidString = liquidString + `hardness: ${0.3 * hardnessAdjustment},\n    viscosity: ${viscosity},\n    breakInto: \"${name}_gas\",\n};\n\n`

      liquidString = liquidString + `elements.${name}_ice = {\n    name: \"${name} ice\",\n    color: \"${colors[1]}\",\n    behavior: behaviors.WALL,\n    tempHigh: ${freezingPoint},\n    temp: ${freezingPoint - 20},\n    stateHigh: \"${name}\",\n    category: \"random solids\",\n    state: \"solid\",\n    density: ${solidDensity},\n    `
      if (conducts == true) {
        liquidString = liquidString + `conduct: ${conductivity},\n    `
      }
      if (burns == true) {
        liquidString = liquidString + `burn: ${burn},\n    burnTime: ${burnTime},\n    `
      }
