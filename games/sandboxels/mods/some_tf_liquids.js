nonAdjacentCoords = [
  ...
  elements.blazing_pyrotheum = {
	  ...
	  tick: function(pixel) {
		if (pixel.temp >= -273 && pixel.temp <= 3707) {
		  pixel.temp += 50
	  ...
	  for (i = 0; i < adjacentCoords.length; i++) {
		...
		if (!isEmpty(fX, fY, true)) {
		  ...
		  if (otherElement.movable) {
			...
			if (Math.random() < 1 / 3) {
			  var randomVxChange = Math.floor(Math.random() * 9) - 4;
			  var randomVyChange = Math.floor(Math.random() * 9) - 4;
  ...
  runAfterLoad(function() {
	...
	if (minecraftModEnabled == true) {
	  ...
	  elements.molten_sterling.reactions.destabilized_redstone = {
		"elem1": null,
		"elem2": "molten_signalum"
	  }
	  ...
	  elements.energized_glowstone.reactions.gelid_cryotheum = {
		"elem1": "glowstone_dust"
	  }
	};
  });