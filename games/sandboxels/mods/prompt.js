...
if (enabledMods.includes(variablesMod)) {
  commandHelpObject = {
    ...
    "worldgen": `Names or sets the current world type, lists all world types, or generates a given world type.
Usages:
...
${eightSpaces}Generate a specified worldgen setting: worldgen generate [setting]`,
    ...
    "defineworldgen": `Creates or replaces a worldgen preset.
...
${eightSpaces}Example full layer definition: 0.85:grass;0.5:dirt;0.05:rock;-0.2:basalt`,
    ...
    "defineworldgen2": `(Optional) baseHeight: Number (ideally between 0 and 1). Default: 0.5.
        ...
        if (property === "color") {
          if (!value.startsWith("rgb(")) {
            if (value.startsWith("hsl(")) {
              if (!(value.split(",")[1].endsWith('%')) || !(value.split(",")[2].endsWith('%)'))) {
          ...
          if (value.startsWith("rgb(")) {
            ...
            var checkedColorObject = hslStringToUnvalidatedObject(value);
...
(Usable area is 1 pixel less in both dimensions)`);
      ...
      case "fill":
        ...
        for (i = 0; i < elementList.length; i++) {
          ...
          var originalElement = elementInConsideration;
      ...
      case "count":
        ...
        var originalInput = inputElement;
...
Make sure to save your command in a file if you want to add this preset again.`
      ...
      case "starseed":
        ...
        return lastStarSeed;
      case "help":
        var commandsWithoutDwg2 = Object.keys(commandHelpObject).filter(function(cmdName) {
          return cmdName !== "defineworldgen2"
        });
        if (inputAsArray.length < 1) {
  ...
  document.addEventListener("keydown", function(e) {
    ...
    if ([1, 3].includes(shiftDown) && e.keyCode == 49) {
      funniPrompt();
    };
  });
