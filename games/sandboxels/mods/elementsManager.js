if (enabledMods.includes("mods/betterMenuScreens.js")) {
    ...
    const exportElements = () => {
      ...
      const blob = new Blob([JSON.stringify(Storage.get("elements", []))], {
    ...
    const elementsManagerLoader = () => {
      ...
      let lastFreeRemoval = Storage.get("settings", {
        allowFreeRemoval: false,
        clearElements: false
      }, true).allowFreeRemoval;
      Storage.set("lastFreeRemoval", lastFreeRemoval);
      for (const key of Object.keys(elements).concat(customElements.map(e => e.name)).sort((a, b) => a.localeCompare(b, undefined, {
          caseFirst: "false"
        }))) {
